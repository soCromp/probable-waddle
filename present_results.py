...
parser.add_argument('--dice', '-d', default=0, type=int, help='percentile of top units included')
...
def main():
    ...
    for test_epoch in args.test_epochs.split():
        ...
        with open(os.path.join(save_dir, f'energy_score_at_epoch_{test_epoch}_d{args.dice}_scoreenv0123.npy'), 'rb') as f:
        ...
        for out_dataset in out_datasets:
            ...
            with open(os.path.join(save_dir, f'energy_score_{out_dataset}_at_epoch_{test_epoch}_d{args.dice}.npy'), 'rb') as f:
        ...
        print_results(s, avg_results, args.in_dataset, "All", args.name, "energy sum", args.test_epochs, args.dice)

# def print_results(s, results, in_dataset, out_dataset, name, method, top, epoch, custom_mask,b,t,env):
def print_results(s, results, in_dataset, out_dataset, name, method, epoch, dice):
    ...
    file = os.path.join('experiments', in_dataset, name, 'result_epoch_{epoch}_d{dice}.txt'.format(epoch=epoch, dice=dice))
